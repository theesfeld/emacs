...
(use-package
 ...
 (defun my-ibuffer-generate-project-groups ()
   ...
   (let ((projects (make-hash-table :test 'equal)))
     ;; Collect all project names from buffers safely
     ...
     (let (groups)
       (maphash
        ...
        projects)
       (nreverse groups)))) ; Reverse to maintain consistent order
 ...
 (defun my-ibuffer-initialize-filter-groups ()
   ...
   (unless my-ibuffer-filter-groups-initialized
     (let ((project-groups (my-ibuffer-generate-project-groups)))
       (setq
       ...
       (when (derived-mode-p 'ibuffer-mode)
         (ibuffer-update nil t))))) ; Force update if in ibuffer
