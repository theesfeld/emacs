...
(use-package
 xlsx
 :ensure t
 :after (all-the-icons consult corfu org)
 :init
 ;; Associate .xlsx files with a custom mode
 (add-to-list 'auto-mode-alist '("\\.xlsx\\'" . xlsx-mode))
 ;; Define a prefix keymap for xlsx commands
 (defvar my-xlsx-prefix-map (make-sparse-keymap)
   "Prefix keymap for xlsx-mode commands.")
 (define-prefix-command 'my-xlsx-prefix-map)
 (global-set-key (kbd "C-c x") 'my-xlsx-prefix-map)
 :config
 ;; Ensure xlsx-mode buffers get an icon
 (with-eval-after-load 'all-the-icons
   (add-to-list
 ...
 ;; Custom function to load and display xlsx as an org-table
 (defun my-xlsx-to-org-table (file)
   "Load an XLSX FILE and display its first sheet as an Org table."
   (interactive "fSelect XLSX file: ")
   (let ((buffer
          (generate-new-buffer
           (format "*XLSX: %s*" (file-name-nondirectory file)))))
     (with-current-buffer buffer
       (xlsx-mode)
       (let* ((sheets (xlsx-read file))
              (first-sheet (car sheets))
              (data (xlsx-sheet-data first-sheet)))
         (erase-buffer)
         (insert "#+TITLE: " (file-name-nondirectory file) "\n\n")
         (insert
          "| "
          (string-join (mapcar #'car (xlsx-sheet-headers first-sheet))
                       " | ")
          " |\n")
         (insert
          "|-"
          (make-string
           (* 2 (length (xlsx-sheet-headers first-sheet))) ?-)
          "-|\n")
         (dolist (row data)
           (insert
            "| "
            (string-join (mapcar (lambda (cell) (or cell "")) row)
                         " | ")
            " |\n"))
         (org-mode)
         (org-table-align)
 ...
 ;; Function to edit xlsx data in org-table and save back
 (defun my-xlsx-edit-and-save ()
   "Edit current Org table buffer as XLSX and save back to original file."
   (interactive)
   (unless (and (eq major-mode 'org-mode) (org-at-table-p))
     (error "Not in an Org table"))
   (let* ((file
           (expand-file-name (file-name-nondirectory (buffer-name))
                             temporary-file-directory))
          (table-data (org-table-to-lisp))
          (headers (car table-data))
          (rows (cdr table-data))
          (xlsx-data
           (mapcar (lambda (row) (mapcar #'identity row)) rows)))
     (xlsx-write
      file
      (list
       (xlsx-make-sheet
        ...
        :headers headers
        :data xlsx-data)))
     (message "Saved edited XLSX to %s" file)
     (find-file file)))

 ;; Export xlsx to other formats via pandoc
 (defun my-xlsx-export (format)
   "Export current XLSX buffer to FORMAT (e.g., csv, html, pdf) using pandoc."
   (interactive (list
                 (completing-read
                  "Export format: " '("csv" "html" "pdf" "ods")
                  nil t)))
   (let* ((input-file (buffer-file-name))
          (output-file
           (concat (file-name-sans-extension input-file) "." format)))
     (unless (and input-file (file-exists-p input-file))
       (error "No XLSX file associated with this buffer"))
     (unless (executable-find "pandoc")
       (error "Pandoc not found; please install it"))
     (shell-command
      (format "pandoc -f xlsx -t %s %s -o %s"
              (if (string= format "pdf")
                  "html"
                format) ; pdf needs intermediate html
              input-file output-file))
     (when (string= format "pdf")
       (shell-command
        (format "wkhtmltopdf %s %s" output-file output-file)))
     (message "Exported to %s" output-file)
     (find-file output-file)))

 ;; Integrate with consult for quick access
 (defun my-consult-xlsx-files ()
   "Search and open XLSX files using consult."
   (interactive)
   (let ((files (directory-files-recursively "~/" "\\.xlsx$" t)))
     (consult--read
      files
      :prompt "Open XLSX file: "
      :category 'file
      :history 'file-name-history
      :action #'my-xlsx-to-org-table)))

 ;; Enable corfu for in-buffer completion within xlsx-mode
 (add-hook 'xlsx-mode-hook #'corfu-mode)

 ;; Keybindings under C-c x
 (define-key my-xlsx-prefix-map (kbd "o") #'my-xlsx-to-org-table)
 (define-key my-xlsx-prefix-map (kbd "e") #'my-xlsx-edit-and-save)
 (define-key my-xlsx-prefix-map (kbd "x") #'my-xlsx-export)
 (define-key my-xlsx-prefix-map (kbd "f") #'my-consult-xlsx-files)
 ...
 :hook
 (xlsx-mode . (lambda () (display-line-numbers-mode -1))) ; Cleaner UI
 :bind
 (:map
  xlsx-mode-map
  ("C-c C-s" . my-xlsx-edit-and-save) ; Quick save in xlsx-mode
  ("C-c C-e" . my-xlsx-export))) ; Quick export in xlsx-mode
